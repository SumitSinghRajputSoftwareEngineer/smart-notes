...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const secPass = await bcrypt.hash(req.body.password, salt); //adding hashing and salt
      //create a new user
...
//Verify the login of a user using : POST "/api/auth/login"..  Doesn't require signup
router.post(
  "/login",
  [
    body("email", "Enter a valid email").isEmail(),
